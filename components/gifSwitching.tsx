...
import styles from './gifSwitching.module.css';
...
export default function GifSwitching(): JSX.Element {
  ...
  return (
    <div className={`page-part ${styles.switch}`} id='usage'>
      <div className={styles.leadIn}>
        <h1 className={styles.heading}>海量丰富功能</h1>
        <p className={styles.subheading}>让求是生活更轻巧的小技巧</p>
      </div>
      <div className={styles.typeChange}>
        <p
          onClick={() => (slider as any).current.slickGoTo(0)}
          className={slideIndex <= 6 ? styles.AndroidActive : styles.Android}>
          Android
        </p>{' '}
        /
        <p
          onClick={() => (slider as any).current.slickGoTo(7)}
          className={slideIndex > 6 ? styles.iOSActive : styles.iOS}>
          iOS
        </p>
      </div>
      <div>
            ...
            <div key={index + 1}>
              <img
                className={`${styles.gif} ${
                  index == (slideIndex == 9 ? 0 : slideIndex + 1)
                    ? ' '
                    : styles.transparency
                }`}
                src={`/gifSwitching/${index == 0 ? 10 : index}.gif`}
              />
              {index == (slideIndex == 9 ? 0 : slideIndex + 1) ? (
